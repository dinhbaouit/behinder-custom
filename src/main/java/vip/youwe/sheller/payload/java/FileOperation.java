...
public class FileOperation {
    ...
    private String create(PageContext page) throws Exception {
        ...
        fso.flush();
        fso.close();
        return path + "Upload complete, remote file size:" + (new File(path)).length();
    }
    ...
    private Map<String, String> renameFile(PageContext page) throws Exception {
        ...
        if (oldFile.exists() && oldFile.isFile() & oldFile.renameTo(newFile)) {
            result.put("status", "success");
            result.put("msg", "renaming completed:" + newPath);
        } else {
            result.put("status", "fail");
            result.put("msg", "Failed to rename:" + newPath);
        }
    ...
    private String append(PageContext page) throws Exception {
        ...
        fso.flush();
        fso.close();
        return path + "Append completed, remote file size:" + (new File(path)).length();
    }
    ...
    private Map<String, String> delete(PageContext page) throws Exception {
        ...
        if (f.exists()) {
            if (f.delete()) {
                result.put("status", "success");
                result.put("msg", path + "successfully deleted.");
            } else {
                result.put("status", "fail");
                result.put("msg", "file" + path + "exists, but failed to delete.");
            }
        } else {
            result.put("status", "fail");
