...
public class PluginTools {
    ...
    public void sendTask(String pluginName, String paramStr) throws Exception {
        ...
        this.statusLabel.setText("Executing plugin...");
        Runnable runner = () -> {
            try {
                ...
                e.printStackTrace();
                Platform.runLater(() -> this.statusLabel.setText("plugin failed to run"));
            }
        };
    ...
    public void getTaskResult(String pluginName) {
        this.statusLabel.setText("Refreshing task execution results......");
        Runnable runner = () -> {
            try {
                ...
                Platform.runLater(() -> {
                    if (status.equals("success")) {
                        this.statusLabel.setText("Result refresh successfully");

                        try {
                            this.pluginWebview.getEngine().executeScript(
                                    String.format("onResult('%s','%s','%s')", status, pluginResult, pluginRunning));
                        } catch (Exception e) {
                            this.statusLabel.setText("The result is refreshed successfully, but the plug-in parsing result failed, please check the plug-in:" + e.getMessage());
                        }
                    } else {
                        this.statusLabel.setText("Result refresh failed");
                    }
                ...
                e.printStackTrace();
                Platform.runLater(() -> this.statusLabel.setText("Result refresh failed:" + e.getMessage()));
            }
        };
