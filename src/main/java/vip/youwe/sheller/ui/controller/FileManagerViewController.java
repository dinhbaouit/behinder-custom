...
public class FileManagerViewController {
    ...
    private void initFileManagerView() throws Exception {
        ...
        TreeItem<String> rootItem = new TreeItem<String>("File System", new ImageView());
        ...
        this.charsetCombo.setItems(FXCollections.observableArrayList("Auto", "GBK", "UTF-8"));
    ...
    private void initCharsetCombo() {
        this.charsetCombo.getSelectionModel().selectedItemProperty().addListener((options, oldValue, newValue) -> {
            String filePath = this.filePathText.getText();
            String charset = newValue.toString().equals("auto")? null: this.charsetCombo.getValue().toString();
    ...
    private void uploadFile() throws Exception {
        ...
        fileChooser.setTitle("Please select the file to upload");
            ...
            try {
                if (fileContent.length <bufSize) {
                    ...
                    if (status.equals("fail")) {
                        Platform.runLater(() -> this.statusLabel.setText("File upload failed:" + msg));
                    ...
                    for (int i = 0; i <blocks.size(); i++) {
                        if (i == 0) {
                            ...
                            Platform.runLater(() -> {
                                if (status.equals("fail")) {
                                    this.statusLabel.setText("File upload failed:" + msg);
                                } else {
                                    this.statusLabel.setText(String.format("Uploading...%skb/%skb", bufSize * currentBlockIndex / 1024,
    ...
    private void rename(String oldFileName, String newFileName) {
        ...
        Runnable runner = () -> {
            try {
                ...
                this.statusLabel.setText("The operation failed:" + e.getMessage());
            }
        };
        Thread workThrad = new Thread(runner);
        this.workList.add(workThrad);
            ...
            try {
                ...
                Platform.runLater(() -> {
                    if (status.equals("success")) {
                        this.statusLabel.setText("Save successfully.");
                    } else {
                        this.statusLabel.setText("Save failed:" + msg);
                    }
    ...
    private void expandByPath(String pathStr) throws UnsupportedEncodingException {
        ...
        this.statusLabel.setText("Loading directory......");
        Runnable runner = () -> {
            try {
                ...
                Platform.runLater(() -> {
                    try {
                        ...
                        String msg = resultObj.getString("msg");
                        if (status.equals("fail")) {
                            this.statusLabel.setText("Failed to read directory:" + msg);
                            return;
                        }

                        this.statusLabel.setText("The catalog is loaded successfully");
                        msg = msg.replace("},]", "}]");
    ...
    private void showFile(String filePath, String charset) {
        Runnable runner = () -> {
            ...
            try {
                ...
                Platform.runLater(() -> {
                    if (status.equals("fail")) {
                        this.statusLabel.setText("File opening failed:" + msg);
                    } else {
    ...
    private void loadContextMenu() {
        ...
        MenuItem downloadBtn = new MenuItem("Download");
        cm.getItems().add(downloadBtn);
        MenuItem uploadBtn = new MenuItem("Upload");
        cm.getItems().add(uploadBtn);

        Menu createMenu = new Menu("New");
        MenuItem createFileBtn = new MenuItem("File...");
        MenuItem createDirectoryBtn = new MenuItem("Folder");
        createMenu.getItems().add(createFileBtn);
        createMenu.getItems().add(createDirectoryBtn);
        cm.getItems().add(createMenu);
        cm.getItems().add(new SeparatorMenuItem());
        MenuItem changeTimeStampBtn = new MenuItem("Modify Timestamp");
        cm.getItems().add(changeTimeStampBtn);
        MenuItem cloneTimeStampBtn = new MenuItem("Clone Timestamp");
        ...
        openBtn.setOnAction(event -> {
            ...
            if (type.equals("directory")) {
                ...
                String charset = this.charsetCombo.getValue().toString().equals("auto")? null: this.charsetCombo.getValue().toString();
                showFile(filePathString, charset);
            }
        });

        refreshBtn.setOnAction(event -> {
            this.statusLabel.setText("Refreshing......");
            try {
                expandByPath(this.currentPathCombo.getValue().toString());
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            this.statusLabel.setText("Refresh complete.");
        });
    ...
    private void downloadFile() {
        ...
        if (selectedFile == null || selectedFile.equals(""))
            return;
        this.statusLabel.setText("Downloading" + fileFullPath + "......");

        Runnable runner = () -> {
            try {
                this.currentShellService.downloadFile(fileFullPath, localFilePath);
                String result = selectedFile.getName() + "download complete, file size:" + selectedFile.length();
