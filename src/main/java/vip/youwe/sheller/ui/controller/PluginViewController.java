...
public class PluginViewController {
    ...
    private void loadPluginDetail(JSONObject pluginObj) {
        ...
        try {
            ...
            this.statusLabel.setText("Plugin developers appreciate that the QR code failed to load");
            e.printStackTrace();
        }
    }
    ...
    private void initPluginInstall() {
        this.installLocalBtn.setOnAction(event -> {
            ...
            fileChooser.setTitle("Please select the plug-in package that needs to be installed");
            ...
            try {
                ...
                if (checkPluginExist(pluginEntity)) {
                    showErrorMessage("error", "installation failed, plugin already exists");
                    return;
                }
                ...
                this.statusLabel.setText("The plugin is installed successfully.");
            } catch (Exception e) {
                e.printStackTrace();
                this.statusLabel.setText("plugin installation failed:" + e.getMessage());
            }
        });
    }
}