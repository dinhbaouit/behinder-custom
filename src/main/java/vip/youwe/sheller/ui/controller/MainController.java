...
public class MainController {
    ...
    public MainController() {
        try {
            ...
            showErrorMessage("error", "database file is missing");
            System.exit(0);
        }
    }
    ...
    private void loadProxy() throws Exception {
        ...
        if (status == Constants.PROXY_ENABLE) {
            ...
            this.proxyStatusLabel.setText("Proxy is in effect");
        }
    }
    ...
    private void initToolbar() {
        this.proxySetupBtn.setOnAction(event -> {
            ...
            RadioButton enableRadio = new RadioButton("Enable");
            RadioButton disableRadio = new RadioButton("disable");
            ...
            try {
                ...
                this.statusLabel.setText("The proxy server configuration failed to load.");
                e.printStackTrace();
            }
    ...
    private boolean checkUrl(String urlString) {
        try {
            ...
            showErrorMessage("error", "URL format error");
            return false;
        }
    }
    ...
    private boolean checkPassword(String password) {
        if (password.length()> 255) {
            showErrorMessage("error", "password length should not be greater than 255 characters");
            return false;
        }
        if (password.length() <1) {

            showErrorMessage("error", "password cannot be empty, please enter password");
            return false;
        }
        return true;
    }
    ...
    private void showShellDialog(int shellID) throws Exception {
        ...
        vpsInfoPane.add(new Label("Password:"), 0, 1);
        vpsInfoPane.add(passText, 1, 1);
        vpsInfoPane.add(new Label("Script Type:"), 0, 2);
        vpsInfoPane.add(shellType, 1, 2);
        vpsInfoPane.add(new Label("Classification:"), 0, 3);
        vpsInfoPane.add(shellCatagory, 1, 3);
        vpsInfoPane.add(new Label("Custom request header:"), 0, 4);
        vpsInfoPane.add(header, 1, 4);
        vpsInfoPane.add(new Label("Remarks:"), 0, 5);
        ...
        saveBtn.setOnAction(e -> {
            ...
            try {
                ...
                e1.printStackTrace();
                showErrorMessage("Save failed", e1.getMessage());
    ...
    private void loadContextMenu() {
        ...
        MenuItem delBtn = new MenuItem("Delete");
        cm.getItems().add(delBtn);
        MenuItem copyBtn = new MenuItem("Copy URL");
        ...
        addBtn.setOnAction(event -> {
            try {
                ...
                showErrorMessage("Error", "Failed to add:" + e.getMessage());
                e.printStackTrace();
            }
        });
        editBtn.setOnAction(event -> {
            ...
            try {
                ...
                showErrorMessage("error", "editing failed:" + e.getMessage());
                e.printStackTrace();
            }
        });
        ...
        delBtn.setOnAction(event -> {
            ...
            alert.setHeaderText("");
            alert.setContentText("Please confirm whether to delete?");
    ...
    private void initCatagoryMenu() {
        ...
        MenuItem addCatagoryBtn = new MenuItem("Add");
        treeContextMenu.getItems().add(addCatagoryBtn);
        MenuItem delCatagoryBtn = new MenuItem("Delete");
        ...
        addCatagoryBtn.setOnAction(event -> {
            ...
            Label cataGoryNameLable = new Label("Please enter the category name:");
            TextField cataGoryNameTxt = new TextField();
            Label cataGoryCommentLable = new Label("Please enter a category description:");
            ...
            if (result.get() == ButtonType.OK) {
                try {
                    if (this.shellManager.addCatagory(cataGoryNameTxt.getText(), cataGoryCommentTxt.getText())> 0) {
                        ...
                        this.statusLabel.setText("Category newly added complete");
                        initCatagoryTree();
                    }
                } catch (Exception e) {
                    this.statusLabel.setText("Failed to add category:" + e.getMessage());
                    e.printStackTrace();
                }
            }
        });
        delCatagoryBtn.setOnAction(event -> {
            ...
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setHeaderText("");
            alert.setContentText("Please confirm whether to delete? Only the category information is deleted, and the website under this category will not be deleted.");
            Optional<ButtonType> result = alert.showAndWait();
            if (result.get() == ButtonType.OK) {
                ...
                try {
                    ...
                    if (this.shellManager.deleteCatagory(cataGoryName)> 0) {
                        ...
                        this.statusLabel.setText("Classification deletion complete");
                        initCatagoryTree();
                    }
                } catch (Exception e) {
                    this.statusLabel.setText("Category deletion failed:" + e.getMessage());
                    e.printStackTrace();
                }
            }
        });
    ...
    private void initCatagoryTree() throws Exception {
        ...
        TreeItem<String> rootItem = new TreeItem<String>("Category List", new ImageView());
